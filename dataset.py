...
class Data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.mode=='train':
            try:
                mask  = cv2.imread(self.cfg.datapath+'/Mask/' +name+'.png', 0)
    ...
    def collate(self, batch):
        ...
        image  = torch.from_numpy(np.stack(image, axis=0)).permute(0,3,1,2).to(torch.float32)
        mask   = torch.from_numpy(np.stack(mask, axis=0)).unsqueeze(1).to(torch.float32)
        return image, mask
